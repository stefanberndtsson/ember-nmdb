<script type="text/x-handlebars" data-template-name="Nmdb-person-page-by_genre-dropdown">
foo
  <option value="">Genres</option>
  {{#each entry in pageData}}
  <option value="{{unbound entry.genre}}"><h4>{{entry.genre}} ({{entry.count}})</h4></option>
  {{/each}}
</script>

<script type="text/x-handlebars" data-template-name="Nmdb-person-page-by_genre">
  <div class="row">
    <div {{bind-attr class="isMobile::panel isMobile::panel-default"}}>
      {{#unless isMobile}}
      <div class="panel-heading">
	<h4 class="panel-title">Movies by Genre</h4>
      </div>
      {{/unless}}
      <div class="col-xs-12 visible-xs visible-sm no-padding">
	<h4>{{view Nmdb.PersonPageByGenreDropdownView}}</h4>
      </div>
      <div class="col-md-3 visible-md visible-lg no-padding">
	<div class="panel-body no-padding-right">
	  <div class="list-group">
	    <h4 class="panel-heading">Genres</h4>
	  {{#each entry in pageData}}
	  <a class="list-group-item" {{action scrollTo entry.genre}}>{{entry.genre}} ({{entry.count}})</a>
	  {{/each}}
	  </div>
	</div>
      </div>
      ...
      {{#each entry in pageData}}
      <div {{bind-attr id="entry.genre" class="isMobile:panel isMobile:panel-default isMobile:margin-bottom-large"}}>
      <div {{bind-attr class=":panel-body isMobile:no-padding"}}>
	<h4 class="panel-heading">{{entry.genre}}</h4>
	{{#unless isMobile}}
